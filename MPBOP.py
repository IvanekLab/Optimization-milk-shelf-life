...
BB    = MILP.addVar ( lb = 0, vtype = GRB.CONTINUOUS, name = 'BB' ) 
...
MILP.addConstr (  quicksum ( FCM * xMF + quicksum ( VCM * NDP[d,p] * xMF for p in Prange ) for d in Drange) + 
                  ...
                  quicksum ( quicksum ( NWP[w,p] * PRpaid[w,p]  for p in Prange ) for w in Wrange ) <= Dsize * BB ) 
